...
#pragma mark - 特别注意视图添加的层次，因为添加的先后顺序影响显示的效果
//从xib中创建了各各控件
- (void)setUI
{
    ...
    //设置背景色,对于控制器的view，其实也是一种懒加载的方式
    self.view.backgroundColor = [UIColor whiteColor];
    
    //初始化最底部的scrollView,装tableView用
   // CGRect frame = CGRectMake(0, 200, WNXAppWidth, WNXAppHeight-200);
   ...
   //self.backgroundScrollView = [[UIScrollView alloc] initWithFrame:frame];
    ...
    //设置可以滚动的区域，这里其实写死了不好。应该根据返回的数据来确定个数
    //注意这里只确定了size，而没有确定x，y。默认就是x = 0 ; y = 0
    ...
    //设置rmdTableView的外边距，因为在之前设置的frame是[UIScreen mainScreen].bounds。
    
#pragma mark - 在创建控件之后再设置frame的具体大小。
    //这种方式不用在初始化的时候确定控件的frame
...
#pragma mark - 把需要显示的视图，放在最后添加
    ...
    //添加顶部view用做topScrollView的父控件，因为在topScrollView内部youpageView应该在同一父控件中，
    //方便后面做拉伸动画
...
#pragma mark -  初始化重写,根据UI从上到下，从做到右的顺序来
//创建子控件
- (void)createSubview{
    //一共三个部分，头，中（应该封装在一起），下
    self.topView = [[UIView alloc]init];
    self.topScrollView = [[WNXScrollHeadView alloc]init];
    [self.topView addSubview:self.topScrollView];
    ...
    self.selectView = [[WNXSelectView alloc]init];
    [self.backgroundScrollView addSubview:self.selectView];
    
    self.footView = [[WNXDetailFootView alloc]init];
    
    self.rmdTableView = [[UITableView alloc]init];
    self.tableHeadView = [[WNXDetailRnmdTableHeadView alloc]init];
    self.rmdTableView.tableFooterView = self.footView;
    [self.rmdTableView addSubview:self.tableHeadView];
    [self.backgroundScrollView addSubview:self.rmdTableView];
    
    self.infoTableView = [[UITableView alloc]init];
    self.mapView = [[MAMapView alloc]init];
    self.infoTableView.tableFooterView = self.footView;
    ...
    self.beenAndCollectView = [[WNXBeenAndCollectView alloc]init];
    [self.view addSubview:self.beenAndCollectView];
    
    
}

//创建子控件frame，由于不是UIview的子类。所以自定义,否则可以用layoutsubivew
- (void)createSubviewFrame{
    self.topScrollView.frame = CGRectMake(0, 0, WNXAppWidth, ScrollHeadViewHeight);
    self.backgroundScrollView.frame = CGRectMake(0, CGRectGetMaxY(self.topScrollView.frame), WNXAppWidth,self.view.bounds.size.height);
    self.selectView.frame = CGRectMake(0, CGRectGetMaxY(self.topScrollView.frame), WNXAppWidth,ScrollHeadViewHeight);
    self.rmdTableView.frame = CGRectMake(0, CGRectGetMaxY(self.selectView.frame), WNXAppWidth, self.backgroundScrollView.frame.size.height);
#warning 未布局完
    
}

//创建子控件的显示的数据
- (void)createSubviewData{
...
//                ischangge = false;
//            }
//            CGAffineTransform tranform = CGAffineTransformMakeTranslation(0,- (initY + offsetY));
//            self.selectView.transform = tranform;
            //self.selectView.transform = CGAffineTransformTranslate(tranform, 0, - (initY + offsetY));
...
#warning 当且仅当切换tableview的时候会走这里
        CGFloat selectViewOffsetY = self.selectView.frame.origin.y - ScrollHeadViewHeight;
