...
//    self.naviView.headModel = self.headModel;
    ...
    self.naviView = [WNXCustomNaviView customNavViewInitWithframe:CGRectMake(0, 0, WNXAppWidth, 64) model:self.headModel];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    //隐藏系统的导航条，由于需要自定义的动画，自定义一个view来代替导航条。隐藏之后但是位置还存在，所以需要覆盖
   [self.navigationController setNavigationBarHidden:YES animated:YES];
    //[self.navigationController setTitle:@"wangxiaolong"];
